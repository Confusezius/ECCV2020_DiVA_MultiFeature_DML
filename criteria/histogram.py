...
class Criterion(torch.nn.Module):
    ...
    def forward(self, batch, labels, **kwargs):
        ...
        sim_inds = torch.triu(torch.ones(similarity.size()), 1).byte().to(self.par.device)
        ...
        hist_pos_inds = torch.tril(torch.ones(hist_pos_rep.size()), -1).byte()
    ...
    def histogram(self, unique_sim_rep, assigned_bin_values, idxs, n_elem):
        ...
        indsa = (assigned_bin_values==(self.support-self.bin_width) ) & idxs.detach().cpu().numpy()
        indsb = (assigned_bin_values==self.support) & idxs.detach().cpu().numpy()
