...
class Criterion(torch.nn.Module):
    ...
    def update_memory_queue(self, embeddings, class_labels):
    ...
    def create_memory_queue(self, model, dataloader, device, opt_key=None):
        with torch.no_grad():
            ...
            self.memory_queue = {i:[] for i in range(self.n_classes)}

            !!
            ### Extract per-class indices from dataset and include them as sample indices!
            !!
