...
class Sampler(torch.utils.data.sampler.Sampler):
    ...
    def precompute_indices(self):
        ...
        self.epoch_indices = Parallel(n_jobs = self.n_jobs)(delayed(self.distmoment_match)(n_calls, i) for i in range(self.n_jobs))
    ...
    def distmoment_match(self, calls, pos):
        """
        """
        coll = []
        ...
        for _ in range(calls):
            ...
            bigb_distmat  = self.get_distmat(bigbatch)
            bigb_dist_mu  = np.mean(bigb_distmat)
            bigb_dist_std = np.std(bigb_distmat)
            ...
            for _ in range(self.num_batch_comps):
                ...
                subset_dist_mu   = np.mean(subset_distmat)
                subset_dist_std  = np.std(subset_distmat)
                subset_mu  = np.mean(subset, axis=0)
                subset_std = np.std(subset, axis=0)


                cost = np.linalg.norm(bigb_mu - subset_mu) + np.linalg.norm(bigb_std - subset_std) + 5*np.sqrt((bigb_dist_mu - subset_dist_mu)**2) + 5*np.sqrt((bigb_dist_std - subset_dist_std)**2)
