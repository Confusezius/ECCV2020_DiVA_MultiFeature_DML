...
class Sampler(torch.utils.data.sampler.Sampler):
    ...
    def __init__(self, opt, image_dict, image_list):
        ...
        self.sim_measure     = opt.data_sim_measure
        self.softened        = opt.data_gc_softened

        self.mb_lr, self.mb_mom = opt.data_mb_lr, opt.data_mb_mom
    ...
    def precompute_indices(self):
        ...
        def batchfinder(n_calls, pos):
            ...
            for i in range(len(structured_batches)):
                class_idx = np.random.choice([self.image_list[idx][-1] for idx in structured_batches[i]])
                structured_batches[i].append(random.choice(self.image_dict[class_idx])[-1])
    ...
    def create_storage(self, dataloader, model, device):
        with torch.no_grad():
            ...
            self.delta_storage = torch.zeros_like(embed_collect)
    ...
    def greedy_coreset(self, calls, pos):
        ...
        for _ in range(calls):
            ...
            if self.sim_measure=='euclidean':
                ...
                dist_matrix          = np.matmul(bigbatch, bigbatch.T)
                coreset_anchor_dists = -1.*np.sum(dist_matrix, axis=1)
            elif self.sim_measure=='square':
                prod        = np.matmul(bigbatch, bigbatch.T)
                sq          = prod.diagonal().reshape(bigbatch.shape[0], 1)
